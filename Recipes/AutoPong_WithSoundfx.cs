...
namespace Game1
{
    public static class AutoPong
    {
        ...
        public static DynamicSoundEffectInstance DSEI;
        ...
        public static void Reset()
        {
            ...
            if(DSEI == null)
            {
                ...
                DSEI.BufferNeeded += new EventHandler<EventArgs>(OnBufferNeeded);
                BufferSize = DSEI.GetSampleSizeInBytes(TimeSpan.FromMilliseconds(200));
        ...
        public static void PlaySoundFx(double freq)
        {
            DSEI.Stop();
            for (int i = 0; i < BufferSize - 1; i += 2)
            {
                double time = (double)TotalTime / (double)SampleRate;
                short currentSample = (short)(Math.Sin(2 * Math.PI * freq * time) * (double)short.MaxValue);
